...
public class Main extends PluginBase implements Listener {
    ...
    public void onMove(PlayerMoveEvent e) {
        ...
        if (p.distance(new Vector3(p.getLevel().getSpawnLocation().getX(), p.getY(), p.getLevel().getSpawnLocation().getZ())) > (p.getLevel().getDimension() == Level.DIMENSION_NETHER ? netherDistance : distance)) {
